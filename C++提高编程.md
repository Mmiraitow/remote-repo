...
```C++
template<typename T> //不指定返回值类型和形参类型
函数声明或定义
```
...
T    ---   通用的数据类型，名称可以替换，通常为大写字母（名字可以改）
...
template<typename T>
void printArray(T arr[], int len) {//打印出来
...
* 如果利用显示指定类型的方式，可以发生隐式类型转换(使用显式指定类型)
...
#### 1.2.5 普通函数与函数模板的调用规则（就是说普通的和模板的都有，优先会调用哪个）
...
```c++
template<typename T>//声明语法跟函数模板一样？ 好像还真是
类
```
...
//类模板对象做函数参数
//1、指定传入的类型
void printPerson1(Person<string, int> &p) //以引用的方式传参
{
	p.showPerson();
}
void test01()
...
**迭代器：**容器和算法之间粘合剂 算法通过迭代器才能访问容器中的元素
...
* 对vector容器的容量和大小操作
* vector容器的容量规定是动态的，它会动态地增长以适应需要，而且可以在必要时收缩
